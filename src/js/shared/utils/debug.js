...
function logViewportDetails() {
  ...
  if (newsletterGrid) {
    ...
    const isHorizontallyOverflowing =
      newsletterGrid.scrollWidth > newsletterGrid.clientWidth;
    const isVerticallyOverflowing =
      newsletterGrid.scrollHeight > newsletterGrid.clientHeight;
  ...
  if (demoGrid) {
    ...
    const isHorizontallyOverflowing =
      demoGrid.scrollWidth > demoGrid.clientWidth;
    const isVerticallyOverflowing =
      demoGrid.scrollHeight > demoGrid.clientHeight;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // CSS Breakpoints
  const breakpoints = {
    "--breakpoint-xs": getComputedStyle(
      document.documentElement,
    ).getPropertyValue("--breakpoint-xs"),
    "--breakpoint-sm": getComputedStyle(
      document.documentElement,
    ).getPropertyValue("--breakpoint-sm"),
    "--breakpoint-md": getComputedStyle(
      document.documentElement,
    ).getPropertyValue("--breakpoint-md"),
    "--breakpoint-lg": getComputedStyle(
      document.documentElement,
    ).getPropertyValue("--breakpoint-lg"),
    "--breakpoint-xl": getComputedStyle(
      document.documentElement,
    ).getPropertyValue("--breakpoint-xl"),
    "--breakpoint-2xl": getComputedStyle(
      document.documentElement,
  ...
  // Media Query States
  const mediaStates = {
    "above-xs": window.matchMedia(
      `(min-width: ${breakpoints["--breakpoint-xs"]})`,
    ).matches,
    "above-sm": window.matchMedia(
      `(min-width: ${breakpoints["--breakpoint-sm"]})`,
    ).matches,
    "above-md": window.matchMedia(
      `(min-width: ${breakpoints["--breakpoint-md"]})`,
    ).matches,
    "above-lg": window.matchMedia(
      `(min-width: ${breakpoints["--breakpoint-lg"]})`,
    ).matches,
    "above-xl": window.matchMedia(
      `(min-width: ${breakpoints["--breakpoint-xl"]})`,
    ).matches,
    "above-2xl": window.matchMedia(
      `(min-width: ${breakpoints["--breakpoint-2xl"]})`,
    ).matches,
  };
  ...
  if (gridContainer) {
    ...
    console.log("Grid Items:", {
      ...
      classes: Array.from(gridItems).map((item) =>
        Array.from(item.classList).filter((c) => c !== "grid-item"),
      ),
      "min-widths": Array.from(gridItems).map(
        (item) => getComputedStyle(item).minWidth,
      ),
      "box-sizing": Array.from(gridItems).map(
        (item) => getComputedStyle(item).boxSizing,
      ),
    });
